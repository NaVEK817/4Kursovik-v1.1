...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                             ...
                             QTabWidget, QSplitter, QFrame, QFileDialog, QDateEdit, QMenu)
...
class RescheduleRequestDialog(QDialog):
    ...
    def show_interview_context_menu(self, pos):
        ...
        interview_id = self.interviews_table.item(row, 0).text()
        date_str = self.calendar.selectedDate().toString("yyyy-MM-dd")
    ...
    def send_invitation_email(self, interview):
        ...
        if not email:
            # Если email не найден, предлагаем ввести
            from PyQt5.QtWidgets import QInputDialog
            email, ok = QInputDialog.getText(
                self,
                "Введите email",
                f"Введите email для кандидата {candidate}:",
                text=""
            )
    ...
    # В демо-версии просто показываем сообщение
        QMessageBox.information(
            ...
            f"✅ Приглашение успешно отправлено на {email}\n\n"
            f"{email_text}"
        )

    def get_candidate_email(self, candidate_name):
        """Получает email кандидата (заглушка - в реальности должна быть БД)"""
    # В демо-версии возвращаем тестовый email
    # В реальности здесь должен быть поиск по БД
        return f"{candidate_name.lower().replace(' ', '.')}@example.com"
